# How to Present the Deck

1. **Open the ready-to-use slideshow**: double-click `chicken_broth_presentation.html` or open it in any modern browser. Use the left/right arrow keys or space bar to advance slides.
2. **Share via Google Slides**: copy the slide content into a new Google Slides file (File → New → Presentation), then apply any theme you like.
3. **Import into PowerPoint or Canva**: paste the markdown content into a new deck and style it with their templates, or export the HTML deck as PDF (Ctrl/Cmd + P → Save as PDF) before uploading.

---

# Slide 1: Title
- **Chicken Broth Masterclass**
- Comforting, nourishing, and easy to prepare

# Slide 2: Why Chicken Broth?
- Rich in minerals and collagen
- Versatile base for soups, sauces, and grains
- Perfect for cold weather comfort

# Slide 3: Ingredients
- 1 whole chicken (or 2 lbs chicken parts)
- 3 carrots, chopped
- 3 celery stalks, chopped
- 1 large onion, quartered
- 4 garlic cloves, smashed
- 2 bay leaves
- 10 peppercorns
- Fresh parsley and thyme sprigs
- 12 cups cold water
- Salt to taste

# Slide 4: Prep Tips
- Rinse chicken and remove giblets
- Roughly chop vegetables—no need to peel
- Tie herbs with kitchen twine for easy removal

# Slide 5: Cooking Steps
1. Place chicken, vegetables, and aromatics in a large stockpot.
2. Cover with cold water and bring to a gentle simmer.
3. Skim foam during the first 20 minutes.
4. Simmer on low for 2.5–3 hours, partially covered.
5. Remove chicken and strain broth through a fine-mesh sieve.

# Slide 6: Finishing Touches
- Cool quickly using an ice bath around the pot
- Refrigerate and remove solidified fat for a lighter broth
- Adjust seasoning with salt and fresh herbs before serving

# Slide 7: Serving Ideas
- Sip warm with a squeeze of lemon
- Use as the base for chicken noodle soup
- Cook grains like rice or quinoa for added flavor
- Freeze in portions for quick weeknight meals

# Slide 8: Presentation Tools
- **PowerPoint**: Classic slide deck experience with rich design options.
- **Google Slides**: Collaborative, browser-based editing.
- **Canva**: Drag-and-drop templates for polished visuals.

# Slide 9: Thank You
- Questions?
- Enjoy your homemade chicken broth!
